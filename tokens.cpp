...
optional<size_t> isEscapedCharacter(char c) {
	string::const_iterator i=find(cEscapedCharacters.begin(),
...
bool looksLikeEmailAddress(const string& str) {
	...
	Iter i=find_if(str.begin(), str.end(), notValidNameCharacter);
	if (i!=str.end() && *i=='@' && i!=str.begin()) {
		...
		if (i==str.end()) {
			...
			RIter ri=find_if(str.rbegin(), str.rend(), isNotAlpha);
			if (ri!=str.rend() && *ri=='.') {
...
void TextHolder::writeAsHtml(ostream& out) const {
	preWrite(out);
	if (mEncodingFlags!=0) {
...
void CodeBlock::writeAsHtml(ostream& out) const {
	out << "<pre><code>";
	TextHolder::writeAsHtml(out);
	out << "</code></pre>\n\n";
}

void CodeSpan::writeAsHtml(ostream& out) const {
	out << "<code>";
	TextHolder::writeAsHtml(out);
	out << "</code>";
}
...
void Container::writeAsHtml(ostream& out) const {
	preWrite(out);
...
void BoldOrItalicMarker::writeAsHtml(ostream& out) const {
	if (!mDisabled) {
		if (mMatch!=0) {
...
void Image::writeAsHtml(ostream& out) const {
