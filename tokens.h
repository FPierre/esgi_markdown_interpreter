...
class Token {
	...
	virtual void writeAsHtml(ostream&) const=0;
	virtual void writeAsOriginal(ostream& out) const { writeAsHtml(out); }
...
class TextHolder: public Token {
	...
	virtual void writeAsHtml(ostream& out) const;
...
class EscapedCharacter: public Token {
	...
	virtual void writeAsHtml(ostream& out) const { out << mChar; }
