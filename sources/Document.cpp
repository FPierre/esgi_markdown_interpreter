...
// namespace {
    ...
    optional<TokenPtr> parseInlineHtml(CTokenGroupIter& i, CTokenGroupIter end) {
        ...
        if ((*i)->text()) {
            ...
            if (tag) {
                ...
                bool done=false;
                do {
                    if ((*i)->text()) contents.push_back(TokenPtr(new InlineHtmlComment(*(*i)->text()+'\n')));
                    else contents.push_back(*i);
            ...
            if (boost::regex_match(line, m, cBlockQuoteExpression)) {
                ...
                while (i!=end) {
                    if ((*i)->isBlankLine()) {
                        ...
                        if (boost::regex_match(line, m, continuationExpression)) {
                            assert(m[2].matched);
                            if (!isBlankLine(m[2])) subTokens.push_back(TokenPtr(new RawText(m[2])));
    ...
    void flushParagraph(string& paragraphText, TokenGroup&
        paragraphTokens, TokenGroup& finalTokens, bool noParagraphs)
    {
